...
ipcMain.handle('scrape:enqueue', async (_e, payload) => {
  const schema = Joi.object({ username: Joi.string().trim().alphanum().min(1).max(30).required() });
...
ipcMain.handle('scrape:bulk', async (_e, payload) => {
  const schema = Joi.object({ usernames: Joi.array().items(Joi.string().trim().alphanum().min(1).max(30)).min(1).max(100).required() });
  const { error, value } = schema.validate(payload);
  if (error) throw new Error(error.message);
  const q = await getQueue();
