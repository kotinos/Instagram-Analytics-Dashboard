...
class DB {
  ...
  addCreator(username, displayName = null) {
    const res = this.stmts.insertCreator.run(username, displayName);
    return Number(res.lastInsertRowid);
  }

  listCreators(limit = 100) {
    return this.stmts.listCreators.all(limit);
  }

  getCreatorByUsername(username) {
    return this.stmts.getCreatorByUsername.get(username) || null;
  }

  touchScraped(id) {
    this.stmts.touchLastScraped.run(id);
  }

  upsertCreatorMeta(meta) {
    return this.stmts.upsertCreatorMeta.run(meta);
  }
}
